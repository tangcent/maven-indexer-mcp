...
function createTestArtifact(version: string = '1.0.0', withSource: boolean = true) {
    ...
    // Create Source
    const srcDir = path.join(TEST_REPO_DIR, `src_tmp_${version}`);
...
describe('Maven Indexer Integration', () => {
    beforeAll(async () => {
        ...
        // Setup Repo (two versions of the same artifact)
        createTestArtifact('1.0.0', true);
        createTestArtifact('2.0.0', false);
    ...
    it('should deduplicate artifacts by groupId:artifactId in class search', async () => {
        const indexer = Indexer.getInstance();
        const results = indexer.searchClass('TestUtils');
        expect(results.length).toBe(1);
        // Both versions exist in DB, but only one artifact per groupId:artifactId should be returned
        expect(results[0].artifacts.length).toBe(1);
        expect(results[0].artifacts[0].groupId).toBe('com.test');
