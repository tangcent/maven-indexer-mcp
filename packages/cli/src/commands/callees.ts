...
export async function run(target: string, opts: { limit?: number } & GlobalOpts): Promise<void> {
  ...
  const edges = indexer.searchCallees(className, methodName, opts.limit);
  ...
  if (!callGraphAvailable) {
    ...
    process.stderr.write(`No callees found for ${label}.\n`);
  }

  print('callees', { target: { className, methodName }, edges, callGraphAvailable }, opts);
}
